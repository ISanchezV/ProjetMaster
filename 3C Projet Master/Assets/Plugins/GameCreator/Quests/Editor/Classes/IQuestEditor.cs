namespace GameCreator.Quests
{
    ...
    public class IQuestEditor : Editor
    {
		...
		private void PaintOnComplete()
        {
			this.editorQuestReactions.PaintOnComplete();
        }

		private void PaintOnFail()
        {
        ...
        public void AddChild(IQuest item, IQuest parent)
        {
            ...
            SerializedObject childrenSerializedObject = new SerializedObject(item);
            childrenSerializedObject.FindProperty(PROP_PARENT).objectReferenceValue = parent;
        ...
        {
            return this.spType.intValue == (int)Task.ProgressType.Incremental;
        }
        ...
        public float GetProgress()
        {
            ...
            {
                Task task = (Task)QuestsManager.Instance.GetQuest(this.target.name);
                if (task != null) return task.progress;
            }
