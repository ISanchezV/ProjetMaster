namespace GameCreator.ModuleManager
{
    ...
    public abstract class ModuleManager
    {
        ...
        private const string MSG_BACKUP_TITLE = "Backup Complete.";
        private const string MSG_BACKUP_INFO = "A Backup file has been generated and safely stored in Project/Backups.";
        ...
        private static string STORE_MODULE_URI = "https://store.gamecreator.io/?search={0}";
        private const string BACKUP_FILEPATH = "Backups/{0}/";
        private const string BACKUP_FILENAME = "{0}_{1}.unitypackage";
        ...
        private static Dictionary<string, Module> STORE_MODULES = null;
        ...
        private static Module[] STORE_MODULES_LIST = null;
        ...
        public static Module[] GetStoreModules()
        {
            if (LOAD_DATA) ModuleManager.InitializeData();
            return ModuleManager.STORE_MODULES_LIST;
        }
        ...
        public static Texture2D GetModuleIcon(string moduleID)
        {
            ...
            if (ModuleManager.PROJECT_MODULES.ContainsKey(moduleID))
            {
                ...
                if (ModuleManager.IsEnabled(manifest.module))
                {
                    ...
                    if (ModuleManager.STORE_MODULES.ContainsKey(moduleID) &&
                        ModuleManager.STORE_MODULES[moduleID].version.Higher(manifest.module.version))
                    {
                        return GetTextureModuleUpdate();
                    }
            ...
            else if (ModuleManager.STORE_MODULES.ContainsKey(moduleID))
            {
                return GetTextureModuleStore();
            }
        ...
        public static void AddStoreModules(Module[] modules)
        {
            ...
            {
                if (modules[i] == null || STORE_MODULES.ContainsKey(modules[i].moduleID)) continue;
                STORE_MODULES.Add(modules[i].moduleID, modules[i]);
            }

            List<Module> modulesList = new List<Module>(STORE_MODULES.Values);
            STORE_MODULES_LIST = modulesList.ToArray();
        ...
        private static void InitializeData()
        {
            ...
            ModuleManager.LoadStoreModules();
        ...
        private static void CallbackUpdateFailed(string packageName, string error)
        {
            AssetDatabase.importPackageFailed -= ModuleManager.CallbackUpdateFailed;
            EditorUtility.DisplayDialog(MSG_UPDATE_FAIL_TITLE, error, "Ok");
        }
        ...
        private static void CallbackEnableFailed(string packageName, string error)
        {
            AssetDatabase.importPackageFailed -= ModuleManager.CallbackEnableFailed;
            EditorUtility.DisplayDialog(MSG_ENABLE_FAIL_TITLE, error, "Ok");
        }
        ...
        private static void ImportModuleContent(
            AssetModule assetModule,
            AssetDatabase.ImportPackageCallback onSuccess,
            AssetDatabase.ImportPackageFailedCallback onFail)
        {
            ...
            if (File.Exists(absolutePackagePath))
            {
                ...
                AssetDatabase.ImportPackage(relativePackagePath, false);
            }
        }
        ...
        public static void OpenStoreSearch(string search)
        {
            Application.OpenURL(string.Format(
                "https://store.gamecreator.io/?search={0}", search.ToLower()
            ));
        }

        public static void OpenStoreTag(string tag)
        {
            Application.OpenURL(string.Format(
                "https://store.gamecreator.io/?tags={0}", tag.ToLower()
            ));
        }
