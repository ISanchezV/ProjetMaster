namespace GameCreator.Characters
{
	...
	public class ActionCharacterProperies : IAction 
	{
            ...
            if (charTarget != null)
            {
                switch (this.changeProperty)
                {
                    case CHANGE_PROPERTY.IsControllable:
                        bool isControllable = this.valueBool.GetValue();
                        charTarget.characterLocomotion.SetIsControllable(isControllable);
                        break;

                    case CHANGE_PROPERTY.CanRun:
                        charTarget.characterLocomotion.canRun = this.valueBool.GetValue();
                        break;

                    case CHANGE_PROPERTY.SetWalkSpeed:
                        charTarget.characterLocomotion.walkSpeed = this.valueNumber.GetValue();
                        break;

                    case CHANGE_PROPERTY.SetRunSpeed:
                        charTarget.characterLocomotion.runSpeed = this.valueNumber.GetValue();
                        break;

                    case CHANGE_PROPERTY.Height:
                        charTarget.characterLocomotion.ChangeHeight(this.valueNumber.GetValue());
                        break;

                    case CHANGE_PROPERTY.JumpForce:
                        charTarget.characterLocomotion.jumpForce = this.valueNumber.GetValue();
                        break;
                }
            }
		...
		private const string NODE_TITLE = "Change {0}'s {1}";
		...
		public override void OnInspectorGUI()
		{
            ...
            switch ((CHANGE_PROPERTY)this.spChangeProperty.intValue)
            {
                ...
                case CHANGE_PROPERTY.SetWalkSpeed: EditorGUILayout.PropertyField(this.spValueNumber); break;
