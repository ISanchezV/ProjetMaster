namespace GameCreator.Characters
{
    ...
    public class CharacterAnimation
    {
        ...
        {
            if (avatarMask == null) this.gesture.PlayGesture(clip, transition);
            else this.gestureMask.PlayGesture(clip, avatarMask, transition);
        }

        public void SetState(AnimationClip clip, AvatarMask avatarMask,
                             float weight, float time, Layer layer)
        {
            if (avatarMask == null)
            {
                this.states[(int)layer].activeState = StateType.Norm;
                this.states[(int)layer].statesNorm.SetState(clip, weight, time);
            ...
            else
            {
                ...
                this.states[(int)layer].statesMask.SetState(clip, avatarMask, weight, time);
            }
        }
                             ...
                             float weight, float time, Layer layer)
        {
            ...
            {
                this.states[(int)layer].activeState = StateType.Norm;
                this.states[(int)layer].statesNorm.SetState(rtc, weight, time);
                this.states[(int)layer].statesMask.ResetState(time);
            }
            else
            {
                this.states[(int)layer].activeState = StateType.Mask;
                this.states[(int)layer].statesNorm.ResetState(time);
                this.states[(int)layer].statesMask.SetState(rtc, avatarMask, weight, time);
            }
        }
