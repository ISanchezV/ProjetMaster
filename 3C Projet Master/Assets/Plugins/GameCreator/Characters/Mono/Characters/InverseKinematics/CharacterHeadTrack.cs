namespace GameCreator.Characters
{
	...
	{
		private const float MOVE_HEAD_SPEED = 1.5f;
		...
		private class TrackInfo
		{
            ...
            private Vector3 currAimDirection = Vector3.zero;
            private Vector3 aimDirectionVelocity = Vector3.zero;
			...
			{
				float targetWeight = this.currentWeight;
				...
				if (nextTrackState == TrackState.NOT_TRACKING)
				{
                    if (this.headTransform != null && this.characterAnimator.useSmartHeadIK)
                    {
                        ...
                        if (currAimDirection == Vector3.zero || Vector3.Angle(aimDirection, currAimDirection) > 90f) 
                        {
                            currAimDirection = aimDirection;
                            this.aimDirectionVelocity = Vector3.zero;
                        }

                        this.currAimDirection = Vector3.SmoothDamp(
                            this.currAimDirection,
                            aimDirection,
                            ref this.aimDirectionVelocity,
                            0.15f
                        );

                        targetPosition = this.headTransform.position + this.currAimDirection;
                    }
                    else
                    {
				...
				float t = Mathf.Clamp01((Time.time - this.changedTime) * MOVE_HEAD_SPEED);
				this.currentPosition = Vector3.Lerp(this.currentPosition, targetPosition, t);
				this.currentWeight = Mathf.Lerp(this.currentWeight, targetWeight, t);
				this.currentTrackState = nextTrackState;
			}

			public void ChangeTrackTarget()
			{
				this.changedTime = Time.time;
			}
		}
		...
		public void Track(Vector3 position)
		{
			...
			this.trackInfo.ChangeTrackTarget();
		}
