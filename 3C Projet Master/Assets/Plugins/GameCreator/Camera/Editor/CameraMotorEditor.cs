namespace GameCreator.Camera
{
	...
	public class CameraMotorEditor : Editor
	{
                ...
                for (int i = 0; i < cameraMotors.Length; ++i)
                {
                    if (cameraMotors[i].GetInstanceID() != curCamMotorID && cameraMotors[i].isMainCameraMotor)
                    {
                        ...
                        if (dialogResult)
                        {
                            ...
                            soOtherCam.ApplyModifiedProperties();
                            soOtherCam.Update();
                        }
                        else
                        {
		...
		private void PaintSelectMotor()
		{
			...
			this.selectMotorIndex = EditorGUILayout.IntPopup(
				this.selectMotorIndex,
				this.cameraMotorsClasses.names,
				this.cameraMotorsClasses.indexes,
